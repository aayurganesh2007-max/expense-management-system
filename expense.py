from database_connections import open_database_connection, close_database_connection, run_db_operation
from analytics import refresh_expense_df
...
def notify_budget_exceeding_expense(date:str, amount:float, category:str, payment_method:str) -> tuple:
    ...
    if valid:
            if amount - balance_amount > 0:
                error_messages.append(f"Adding this expense exceeds the budget for category '{category}'.By amount'{amount-balance_amount}'")    
    valid2,balance_amount2= balance_budget(year,month,payment_method=payment_method)
    if valid2:
            if amount - balance_amount2 > 0:
                error_messages.append(f"Adding this expense exceeds the budget for payment method '{payment_method}'.By amount'{amount-balance_amount2}'")
    valid3,balance_amount3= balance_budget(year,month)
    if valid3:
            if amount - balance_amount3 > 0:
                error_messages.append(f"Adding this expense exceeds the overall monthly budget.By amount'{amount-balance_amount3}'")
    if error_messages:  
...
def add_expense(date:str, amount:float, category:str, description:str, payment_method:str,approval:bool = False)-> tuple:
    ...
    try:
        #with ensures the database connection is closed after the operation
